...
- [ ] **Closing the proposal loop** — Replace the current PR-with-code approach with GitHub Issues tagged `enhancement`. When Mira wants a new capability she opens an issue describing the *desired behaviour and why*, not code. Andrew and Claude pick it up, discuss in the thread, implement if valuable, close with a reason if not. This separates ideation (Mira) from implementation (humans), which is more honest about what she can do. Issues have a natural lifecycle; PRs with dead code don't. Also: feed open issues back into Mira's morning context so she doesn't re-propose the same things. Retire the `proposals/` directory once this is wired up.
